...
--
-- Idempotent via WHERE NOT EXISTS guards (no PL/pgSQL DO block - some SQL
-- editors, including Lakebase's built-in one, don't reliably split
-- statements inside a dollar-quoted DO $$ ... $$ block). Safe to re-run:
-- each INSERT only fires if its target table is still empty.
...
SELECT * FROM (
    VALUES
        ...
        ((SELECT ticket_id FROM tickets WHERE title = 'Export to CSV missing timezone info'),
         ...
         'dave@example.com')
) AS seed(ticket_id, message_text, author)
