...
The URL itself is never stored in code or app.yaml - it's fetched at
runtime from a Databricks secret scope (see setup_secrets.py).
"""
...
def run_write_returning(sql: str, params: tuple | dict | None = None) -> list[dict]:
    """Run an INSERT/UPDATE with a RETURNING clause, commit, and return rows as list[dict]."""
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, params)
            rows = cur.fetchall()
